...
| `failed to bind host port 0.0.0.0:80/tcp: address already in use` (or you see another web server's default page at `http://<host>:80`) | Another process holds port `:80` (most often system apache2/nginx - this stack needs `:80` for Caddy). Confirm with `sudo ss -tlnp \| grep :80`, then stop/disable the other server: `sudo systemctl stop apache2 && sudo systemctl disable apache2` (replace `apache2` with `nginx` or whatever `ss` reports), `sudo pkill -f apache2` to clear lingering workers, then recreate Caddy: `docker compose up -d --remove-orphans`. Re-check `sudo ss -tlnp \| grep :80` shows `docker-proxy`/Caddy, not the web server. On the browser, hard refresh (`Ctrl+Shift+R`) to drop a cached old page |
